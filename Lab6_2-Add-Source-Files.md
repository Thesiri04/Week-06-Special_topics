...
- เพื่อจัดการโค้ดให้เป็นระเบียบ แยกหน้าที่ ลดความซับซ้อน และง่ายต่อการบำรุงรักษา
2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
- ต้องเพิ่มชื่อไฟล์ซอร์สโค้ดใหม่ในส่วน SRCS เพื่อให้ build system รู้จักและคอมไพล์ไฟล์นั้น
3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
- ประกาศฟังก์ชันและโครงสร้างข้อมูล เพื่อให้ไฟล์ซอร์สอื่น ๆ รู้จักและใช้งานร่วมกันได้
4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
- เพื่อบอก compiler ว่าจะหาไฟล์ header (.h) จากโฟลเดอร์ไหน
5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
- ช่วยป้องกันไม่ให้ไฟล์ที่ไม่จำเป็นหรือไฟล์ที่สร้างอัตโนมัติ (เช่น ไฟล์ build) ถูกเพิ่มใน repository
6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
- ช่วยให้ LED กะพริบเป็นงานแยก ไม่บล็อกงานอื่น ทำให้ระบบทำงานพร้อมกันได้อย่างราบรื่น
7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
- ทำให้โค้ดอ่านง่าย แยกความรับผิดชอบชัดเจน และพัฒนาหรือแก้ไขแต่ละโมดูลได้สะดวก
## 📋 ผลลัพธ์ที่คาดหวัง
...
- จำนวนไฟล์ source: 4
- ขนาด binary: 165301 bytes
- การทำงาน: เซนเซอร์ หน้าจอแสดงผล และ LED โดยอ่านข้อมูลอุณหภูมิและความชื้น แสดงผลข้อมูลและสถานะ LED บนจอ พร้อมให้ LED กะพริบทุก 3 วินาที
