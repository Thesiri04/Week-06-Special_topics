...
- Docker: ไม่ต้องติดตั้ง ESP-IDF และ dependencies ในเครื่องตัวเอง, สภาพแวดล้อมพัฒนาเหมือนกันทุกคน, ย้ายไปเครื่องอื่นง่าย
- Native: รันเร็วกว่าเล็กน้อย, เข้าถึง hardware ตรง ๆ ง่าย, แต่ต้องติดตั้งและจัดการเวอร์ชันเอง
2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
- เข้า container → source $IDF_PATH/export.sh ตั้งค่า environment
- idf.py set-target esp32 (ครั้งแรกเท่านั้น)
- idf.py build → CMake สร้าง build system
- Compiler แปลง source .c → object .o
- Linker รวมเป็น .elf
- สร้าง binary .bin, partition table, bootloader
3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
- Top-level CMakeLists.txt → กำหนดชื่อโปรเจกต์, เรียก project.cmake
- main/CMakeLists.txt → ระบุ source files, include dirs ของ component
- ใน Docker ทำงานเหมือน native ต่างกันแค่รันใน container
4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
- กันไฟล์ build output, binary, cache, config ชั่วคราว ไม่ให้เข้า git
- ทำให้ repo สะอาด, ขนาดเล็ก, clone/build ได้ทุกเครื่อง
5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
- หายไปเมื่อ restart container: ไฟล์ที่อยู่ใน container โดยไม่ mount (เช่น /tmp หรือโฟลเดอร์ build ถ้าไม่ได้ map)
- อยู่ต่อ: ไฟล์ในโฟลเดอร์ที่ mount กับ host (เช่น /project ที่ผูกกับ directory บนเครื่องเรา)
6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
- Docker: clone code → เข้า container → build/run ได้ทันที, ไม่ต้องติดตั้งอะไรใน host
- Native: ต้องติดตั้ง ESP-IDF + dependencies ก่อน → build/run บน host, เสี่ยงเจอปัญหาเวอร์ชันไม่ตรงกัน
## 📋 ผลลัพธ์ที่คาดหวัง
